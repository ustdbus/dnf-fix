// 鬼剑士单机版 装备附魔 / 魔法词条 (Option Table) 核心字典
// 逆向基准：libBNVModule.so drawStrInt2DOption (0x00061710 ~ 0x00061b90)、setOptionValue (0x000ad685) 与 loadMagicTable (0x000aa358 / 3.etc)

export interface EnchantParamDef {
  label: string
  min: number
  max: number
  defaultVal: number
  unit?: string
  step?: number
}

export type EnchantCategory = 'element' | 'stat' | 'abnormal' | 'trigger' | 'special'

export interface EnchantDefinition {
  code: number
  name: string
  category: EnchantCategory
  params: EnchantParamDef[]
  format: (p1: number, p2: number, p3: number) => string
}

export type EquipPart = 'weapon' | 'shoulder' | 'top' | 'bottom' | 'bracelet' | 'ring'

export interface EnchantPreset {
  id: string
  name: string
  desc: string
  code: number
  param1: number
  param2: number
  param3: number
  icon: string
  parts?: EquipPart[]
  quality?: 'white' | 'blue' | 'purple' | 'pink' | 'orange'
}

export const ENCHANT_CATEGORIES: { id: EnchantCategory; name: string; icon: string }[] = [
  { id: 'element', name: '属性与属强', icon: '🔥' },
  { id: 'stat', name: '基础与四维', icon: '⚔️' },
  { id: 'special', name: '增伤与爆伤', icon: '✨' },
  { id: 'abnormal', name: '异常状态', icon: '⚡' },
  { id: 'trigger', name: '概率触发', icon: '🎲' }
]

// 全量附魔词条映射表 (严格基于引擎底层 setOptionValue 与 equipText 真实支持的 Option Code)
export const ENCHANT_DEFINITIONS: Record<number, EnchantDefinition> = {
  // --- 0x01 ~ 0x04 基础四维 ---
  0x01: {
    code: 0x01,
    name: '力量',
    category: 'stat',
    params: [{ label: '增加力量', min: 1, max: 9999, defaultVal: 20, unit: '点' }],
    format: (p1) => `力量 +${p1}`
  },
  0x02: {
    code: 0x02,
    name: '智力',
    category: 'stat',
    params: [{ label: '增加智力', min: 1, max: 9999, defaultVal: 20, unit: '点' }],
    format: (p1) => `智力 +${p1}`
  },
  0x03: {
    code: 0x03,
    name: '体力',
    category: 'stat',
    params: [{ label: '增加体力', min: 1, max: 9999, defaultVal: 20, unit: '点' }],
    format: (p1) => `体力 +${p1}`
  },
  0x04: {
    code: 0x04,
    name: '精神',
    category: 'stat',
    params: [{ label: '增加精神', min: 1, max: 9999, defaultVal: 20, unit: '点' }],
    format: (p1) => `精神 +${p1}`
  },

  // --- 0x05 ~ 0x09 血蓝与恢复 (0x09 为真实 HP、MP恢复) ---
  0x05: {
    code: 0x05,
    name: 'HP 最大值',
    category: 'stat',
    params: [{ label: '增加HP上限', min: 1, max: 65535, defaultVal: 200, unit: '点' }],
    format: (p1) => `HP MAX +${p1}`
  },
  0x06: {
    code: 0x06,
    name: 'MP 最大值',
    category: 'stat',
    params: [{ label: '增加MP上限', min: 1, max: 65535, defaultVal: 100, unit: '点' }],
    format: (p1) => `MP MAX +${p1}`
  },
  0x07: {
    code: 0x07,
    name: 'HP 每分钟恢复',
    category: 'stat',
    params: [{ label: '每分钟恢复量', min: 1, max: 9999, defaultVal: 10, unit: '点' }],
    format: (p1) => `HP恢复 +${p1}`
  },
  0x08: {
    code: 0x08,
    name: 'MP 每分钟恢复',
    category: 'stat',
    params: [{ label: '每分钟恢复量', min: 1, max: 9999, defaultVal: 10, unit: '点' }],
    format: (p1) => `MP恢复 +${p1}`
  },
  0x09: {
    code: 0x09,
    name: 'HP、MP恢复',
    category: 'stat',
    params: [{ label: '恢复量', min: 1, max: 9999, defaultVal: 15, unit: '点' }],
    format: (p1) => `HP、MP恢复 +${p1}`
  },

  // --- 0x0A ~ 0x0E 攻防与追加伤害 (0x0E 官方真实为追加伤害白字) ---
  0x0a: {
    code: 0x0a,
    name: '物理攻击力',
    category: 'stat',
    params: [{ label: '物理攻击力', min: 1, max: 9999, defaultVal: 50, unit: '点' }],
    format: (p1) => `物理攻击力 +${p1}`
  },
  0x0b: {
    code: 0x0b,
    name: '魔法攻击力',
    category: 'stat',
    params: [{ label: '魔法攻击力', min: 1, max: 9999, defaultVal: 50, unit: '点' }],
    format: (p1) => `魔法攻击力 +${p1}`
  },
  0x0c: {
    code: 0x0c,
    name: '物理防御力',
    category: 'stat',
    params: [{ label: '物理防御力', min: 1, max: 9999, defaultVal: 100, unit: '点' }],
    format: (p1) => `物理防御力 +${p1}`
  },
  0x0d: {
    code: 0x0d,
    name: '魔法防御力',
    category: 'stat',
    params: [{ label: '魔法防御力', min: 1, max: 9999, defaultVal: 100, unit: '点' }],
    format: (p1) => `魔法防御力 +${p1}`
  },
  0x0e: {
    code: 0x0e,
    name: '追加伤害',
    category: 'special',
    params: [{ label: '追加伤害数值', min: 1, max: 65535, defaultVal: 180, unit: '点' }],
    format: (p1) => `追加伤害 +${p1}`
  },

  // --- 0x0F ~ 0x13 暴击、硬直、速度与命中回避 (0x0F 为全暴击率，0x10 为硬直恢复) ---
  0x0f: {
    code: 0x0f,
    name: '暴击率',
    category: 'stat',
    params: [{ label: '暴击率', min: 1, max: 100, defaultVal: 10, unit: '%' }],
    format: (p1) => `暴击率 +${p1}%`
  },
  0x10: {
    code: 0x10,
    name: '硬直恢复',
    category: 'stat',
    params: [{ label: '硬直恢复', min: 1, max: 9999, defaultVal: 60, unit: '点' }],
    format: (p1) => `硬直恢复 +${p1}`
  },
  0x11: {
    code: 0x11,
    name: '移动速度',
    category: 'stat',
    params: [{ label: '移动速度', min: 1, max: 100, defaultVal: 5, unit: '%' }],
    format: (p1) => `移动速度 +${p1}%`
  },
  0x12: {
    code: 0x12,
    name: '命中率',
    category: 'stat',
    params: [{ label: '命中率', min: 1, max: 100, defaultVal: 5, unit: '%' }],
    format: (p1) => `命中率 +${p1}%`
  },
  0x13: {
    code: 0x13,
    name: '回避率',
    category: 'stat',
    params: [{ label: '回避率', min: 1, max: 100, defaultVal: 5, unit: '%' }],
    format: (p1) => `回避率 +${p1}%`
  },

  // --- 0x14 ~ 0x17 单属性抗性 ---
  0x14: {
    code: 0x14,
    name: '火属性抗性',
    category: 'element',
    params: [{ label: '火属性抗性', min: 1, max: 100, defaultVal: 10, unit: '点' }],
    format: (p1) => `火属性抗性 +${p1}`
  },
  0x15: {
    code: 0x15,
    name: '冰属性抗性',
    category: 'element',
    params: [{ label: '冰属性抗性', min: 1, max: 100, defaultVal: 10, unit: '点' }],
    format: (p1) => `冰属性抗性 +${p1}`
  },
  0x16: {
    code: 0x16,
    name: '光属性抗性',
    category: 'element',
    params: [{ label: '光属性抗性', min: 1, max: 100, defaultVal: 10, unit: '点' }],
    format: (p1) => `光属性抗性 +${p1}`
  },
  0x17: {
    code: 0x17,
    name: '暗属性抗性',
    category: 'element',
    params: [{ label: '暗属性抗性', min: 1, max: 100, defaultVal: 10, unit: '点' }],
    format: (p1) => `暗属性抗性 +${p1}`
  },

  // --- 0x18 技能等级 ---
  0x18: {
    code: 0x18,
    name: '技能等级增加',
    category: 'special',
    params: [
      { label: '技能ID', min: 0, max: 255, defaultVal: 1 },
      { label: '提升等级', min: 1, max: 10, defaultVal: 1, unit: '级' }
    ],
    format: (p1, p2) => `技能 [ID:${p1}] 等级 +${p2}`
  },

  // --- 0x19 ~ 0x24 攻击时触发异常状态 ---
  0x19: {
    code: 0x19,
    name: '攻击时触发出血',
    category: 'abnormal',
    params: [
      { label: '触发几率', min: 1, max: 100, defaultVal: 5, unit: '%' },
      { label: '异常等级', min: 1, max: 100, defaultVal: 30, unit: '级' },
      { label: '持续时间', min: 1, max: 60, defaultVal: 5, unit: '秒' }
    ],
    format: (p1, p2) => `攻击时${p1}%几率造成Lv${p2}出血`
  },
  0x1a: {
    code: 0x1a,
    name: '攻击时触发中毒',
    category: 'abnormal',
    params: [
      { label: '触发几率', min: 1, max: 100, defaultVal: 5, unit: '%' },
      { label: '异常等级', min: 1, max: 100, defaultVal: 30, unit: '级' },
      { label: '持续时间', min: 1, max: 60, defaultVal: 5, unit: '秒' }
    ],
    format: (p1, p2) => `攻击时${p1}%几率造成Lv${p2}中毒`
  },
  0x1b: {
    code: 0x1b,
    name: '攻击时触发灼伤',
    category: 'abnormal',
    params: [
      { label: '触发几率', min: 1, max: 100, defaultVal: 5, unit: '%' },
      { label: '异常等级', min: 1, max: 100, defaultVal: 30, unit: '级' },
      { label: '持续时间', min: 1, max: 60, defaultVal: 5, unit: '秒' }
    ],
    format: (p1, p2) => `攻击时${p1}%几率造成Lv${p2}灼伤`
  },
  0x1c: {
    code: 0x1c,
    name: '攻击时触发减速',
    category: 'abnormal',
    params: [
      { label: '触发几率', min: 1, max: 100, defaultVal: 5, unit: '%' },
      { label: '异常等级', min: 1, max: 100, defaultVal: 30, unit: '级' },
      { label: '持续时间', min: 1, max: 60, defaultVal: 5, unit: '秒' }
    ],
    format: (p1, p2) => `攻击时${p1}%几率造成Lv${p2}减速`
  },
  0x1d: {
    code: 0x1d,
    name: '攻击时触发感电',
    category: 'abnormal',
    params: [
      { label: '触发几率', min: 1, max: 100, defaultVal: 5, unit: '%' },
      { label: '异常等级', min: 1, max: 100, defaultVal: 30, unit: '级' },
      { label: '持续时间', min: 1, max: 60, defaultVal: 5, unit: '秒' }
    ],
    format: (p1, p2) => `攻击时${p1}%几率造成Lv${p2}感电`
  },
  0x1e: {
    code: 0x1e,
    name: '攻击时触发黑暗',
    category: 'abnormal',
    params: [
      { label: '触发几率', min: 1, max: 100, defaultVal: 5, unit: '%' },
      { label: '异常等级', min: 1, max: 100, defaultVal: 30, unit: '级' },
      { label: '持续时间', min: 1, max: 60, defaultVal: 5, unit: '秒' }
    ],
    format: (p1, p2) => `攻击时${p1}%几率造成Lv${p2}黑暗`
  },
  0x1f: {
    code: 0x1f,
    name: '攻击时触发眩晕',
    category: 'abnormal',
    params: [
      { label: '触发几率', min: 1, max: 100, defaultVal: 5, unit: '%' },
      { label: '异常等级', min: 1, max: 100, defaultVal: 30, unit: '级' },
      { label: '持续时间', min: 1, max: 60, defaultVal: 3, unit: '秒' }
    ],
    format: (p1, p2) => `攻击时${p1}%几率造成Lv${p2}眩晕`
  },
  0x20: {
    code: 0x20,
    name: '攻击时触发冰冻',
    category: 'abnormal',
    params: [
      { label: '触发几率', min: 1, max: 100, defaultVal: 5, unit: '%' },
      { label: '异常等级', min: 1, max: 100, defaultVal: 30, unit: '级' },
      { label: '持续时间', min: 1, max: 60, defaultVal: 3, unit: '秒' }
    ],
    format: (p1, p2) => `攻击时${p1}%几率造成Lv${p2}冰冻`
  },
  0x21: {
    code: 0x21,
    name: '攻击时触发石化',
    category: 'abnormal',
    params: [
      { label: '触发几率', min: 1, max: 100, defaultVal: 5, unit: '%' },
      { label: '异常等级', min: 1, max: 100, defaultVal: 30, unit: '级' },
      { label: '持续时间', min: 1, max: 60, defaultVal: 3, unit: '秒' }
    ],
    format: (p1, p2) => `攻击时${p1}%几率造成Lv${p2}石化`
  },
  0x22: {
    code: 0x22,
    name: '攻击时触发束缚',
    category: 'abnormal',
    params: [
      { label: '触发几率', min: 1, max: 100, defaultVal: 5, unit: '%' },
      { label: '异常等级', min: 1, max: 100, defaultVal: 30, unit: '级' },
      { label: '持续时间', min: 1, max: 60, defaultVal: 4, unit: '秒' }
    ],
    format: (p1, p2) => `攻击时${p1}%几率造成Lv${p2}束缚`
  },
  0x23: {
    code: 0x23,
    name: '攻击时触发混乱',
    category: 'abnormal',
    params: [
      { label: '触发几率', min: 1, max: 100, defaultVal: 5, unit: '%' },
      { label: '异常等级', min: 1, max: 100, defaultVal: 30, unit: '级' },
      { label: '持续时间', min: 1, max: 60, defaultVal: 5, unit: '秒' }
    ],
    format: (p1, p2) => `攻击时${p1}%几率造成Lv${p2}混乱`
  },

  // --- 0x25 ~ 0x2F 异常抗性 ---
  0x25: { code: 0x25, name: '出血抗性', category: 'abnormal', params: [{ label: '出血抗性', min: 1, max: 100, defaultVal: 10, unit: '%' }], format: (p1) => `出血抗性 +${p1}%` },
  0x26: { code: 0x26, name: '中毒抗性', category: 'abnormal', params: [{ label: '中毒抗性', min: 1, max: 100, defaultVal: 10, unit: '%' }], format: (p1) => `中毒抗性 +${p1}%` },
  0x27: { code: 0x27, name: '灼伤抗性', category: 'abnormal', params: [{ label: '灼伤抗性', min: 1, max: 100, defaultVal: 10, unit: '%' }], format: (p1) => `灼伤抗性 +${p1}%` },
  0x28: { code: 0x28, name: '感电抗性', category: 'abnormal', params: [{ label: '感电抗性', min: 1, max: 100, defaultVal: 10, unit: '%' }], format: (p1) => `感电抗性 +${p1}%` },
  0x29: { code: 0x29, name: '黑暗抗性', category: 'abnormal', params: [{ label: '黑暗抗性', min: 1, max: 100, defaultVal: 10, unit: '%' }], format: (p1) => `黑暗抗性 +${p1}%` },
  0x2a: { code: 0x2a, name: '眩晕抗性', category: 'abnormal', params: [{ label: '眩晕抗性', min: 1, max: 100, defaultVal: 10, unit: '%' }], format: (p1) => `眩晕抗性 +${p1}%` },
  0x2b: { code: 0x2b, name: '冰冻抗性', category: 'abnormal', params: [{ label: '冰冻抗性', min: 1, max: 100, defaultVal: 10, unit: '%' }], format: (p1) => `冰冻抗性 +${p1}%` },
  0x2c: { code: 0x2c, name: '石化抗性', category: 'abnormal', params: [{ label: '石化抗性', min: 1, max: 100, defaultVal: 10, unit: '%' }], format: (p1) => `石化抗性 +${p1}%` },
  0x2d: { code: 0x2d, name: '束缚抗性', category: 'abnormal', params: [{ label: '束缚抗性', min: 1, max: 100, defaultVal: 10, unit: '%' }], format: (p1) => `束缚抗性 +${p1}%` },
  0x2e: { code: 0x2e, name: '混乱抗性', category: 'abnormal', params: [{ label: '混乱抗性', min: 1, max: 100, defaultVal: 10, unit: '%' }], format: (p1) => `混乱抗性 +${p1}%` },
  0x2f: { code: 0x2f, name: '减速抗性', category: 'abnormal', params: [{ label: '减速抗性', min: 1, max: 100, defaultVal: 10, unit: '%' }], format: (p1) => `减速抗性 +${p1}%` },

  // --- 0x37 削血神器 ---
  0x37: {
    code: 0x37,
    name: '攻击时削减敌人HP (削血)',
    category: 'special',
    params: [
      { label: '触发几率', min: 1, max: 100, defaultVal: 3, unit: '%' },
      { label: '削减百分比', min: 1, max: 100, defaultVal: 20, unit: '%' }
    ],
    format: (p1, p2) => `攻击时${p1}%几率削减敌人当前 HP ${p2}%`
  },
  0x38: {
    code: 0x38,
    name: '攻击时恢复 HP',
    category: 'trigger',
    params: [
      { label: '触发几率', min: 1, max: 100, defaultVal: 5, unit: '%' },
      { label: '恢复数值', min: 1, max: 9999, defaultVal: 30, unit: '点' }
    ],
    format: (p1, p2) => `攻击时${p1}%几率恢复 HP ${p2}`
  },
  0x39: {
    code: 0x39,
    name: '攻击时恢复 MP',
    category: 'trigger',
    params: [
      { label: '触发几率', min: 1, max: 100, defaultVal: 5, unit: '%' },
      { label: '恢复数值', min: 1, max: 9999, defaultVal: 30, unit: '点' }
    ],
    format: (p1, p2) => `攻击时${p1}%几率恢复 MP ${p2}`
  },

  // --- 0x4A ~ 0x4F 攻击时触发属性增强 (三参数: 几率, 持续秒, 数值) ---
  0x4a: {
    code: 0x4a,
    name: '攻击时提升物攻',
    category: 'trigger',
    params: [
      { label: '触发几率', min: 1, max: 100, defaultVal: 3, unit: '%' },
      { label: '持续时间', min: 1, max: 300, defaultVal: 100, unit: '秒' },
      { label: '提升物攻', min: 1, max: 9999, defaultVal: 100, unit: '点' }
    ],
    format: (p1, p2, p3) => `攻击时${p1}%几率${p2}秒物攻+${p3}`
  },
  0x4b: {
    code: 0x4b,
    name: '攻击时提升魔攻',
    category: 'trigger',
    params: [
      { label: '触发几率', min: 1, max: 100, defaultVal: 3, unit: '%' },
      { label: '持续时间', min: 1, max: 300, defaultVal: 100, unit: '秒' },
      { label: '提升魔攻', min: 1, max: 9999, defaultVal: 100, unit: '点' }
    ],
    format: (p1, p2, p3) => `攻击时${p1}%几率${p2}秒魔攻+${p3}`
  },
  0x4c: {
    code: 0x4c,
    name: '被击时提升物防',
    category: 'trigger',
    params: [
      { label: '触发几率', min: 1, max: 100, defaultVal: 2, unit: '%' },
      { label: '持续时间', min: 1, max: 300, defaultVal: 100, unit: '秒' },
      { label: '提升物防', min: 1, max: 9999, defaultVal: 300, unit: '点' }
    ],
    format: (p1, p2, p3) => `被击时${p1}%几率${p2}秒物防+${p3}`
  },
  0x4f: {
    code: 0x4f,
    name: '攻击时提升暴击率',
    category: 'trigger',
    params: [
      { label: '触发几率', min: 1, max: 100, defaultVal: 3, unit: '%' },
      { label: '持续时间', min: 1, max: 300, defaultVal: 20, unit: '秒' },
      { label: '提升暴击率', min: 1, max: 100, defaultVal: 10, unit: '%' }
    ],
    format: (p1, p2, p3) => `攻击时${p1}%几率${p2}秒暴击+${p3}%`
  },

  // --- 0x5F, 0x60, 0x65, 0x6D 官方核心增益 (0x60 为四维全属性增加) ---
  0x5f: {
    code: 0x5f,
    name: '白字附加伤害(百分比)',
    category: 'special',
    params: [{ label: '附加伤害比例', min: 1, max: 100, defaultVal: 5, unit: '%' }],
    format: (p1) => `攻击时附加伤害 +${p1}%`
  },
  0x60: {
    code: 0x60,
    name: '全属性增加',
    category: 'stat',
    params: [{ label: '全属性数值', min: 1, max: 9999, defaultVal: 15, unit: '点' }],
    format: (p1) => `全属性增加 +${p1}`
  },
  0x65: {
    code: 0x65,
    name: '所有属性抗性',
    category: 'element',
    params: [{ label: '所有属性抗性', min: 1, max: 100, defaultVal: 10, unit: '点' }],
    format: (p1) => `所有属性抗性 +${p1}`
  },
  0x6d: {
    code: 0x6d,
    name: '暴击伤害增加',
    category: 'special',
    params: [{ label: '暴击伤害', min: 1, max: 100, defaultVal: 20, unit: '%' }],
    format: (p1) => `暴击伤害增加 +${p1}%`
  },

  // --- 0x79 ~ 0x7C 武器属性赋予 ---
  0x79: {
    code: 0x79,
    name: '赋予武器属性：火属性攻击',
    category: 'element',
    params: [],
    format: () => '赋予武器属性：火属性攻击'
  },
  0x7a: {
    code: 0x7a,
    name: '赋予武器属性：冰属性攻击',
    category: 'element',
    params: [],
    format: () => '赋予武器属性：冰属性攻击'
  },
  0x7b: {
    code: 0x7b,
    name: '赋予武器属性：光属性攻击',
    category: 'element',
    params: [],
    format: () => '赋予武器属性：光属性攻击'
  },
  0x7c: {
    code: 0x7c,
    name: '赋予武器属性：暗属性攻击',
    category: 'element',
    params: [],
    format: () => '赋予武器属性：暗属性攻击'
  },

  // --- 0x7D 概率额外追加伤害 ---
  0x7d: {
    code: 0x7d,
    name: '攻击时概率追加伤害',
    category: 'trigger',
    params: [
      { label: '触发几率', min: 1, max: 100, defaultVal: 3, unit: '%' },
      { label: '追加伤害', min: 1, max: 65535, defaultVal: 100, unit: '点' }
    ],
    format: (p1, p2) => `攻击时${p1}%几率追加伤害${p2}`
  },

  // --- 0x7E ~ 0x7F 百分比血蓝上限 ---
  0x7e: {
    code: 0x7e,
    name: 'HP MAX (百分比)',
    category: 'stat',
    params: [{ label: '生命上限比例', min: 1, max: 100, defaultVal: 10, unit: '%' }],
    format: (p1) => `HP MAX +${p1}%`
  },
  0x7f: {
    code: 0x7f,
    name: 'MP MAX (百分比)',
    category: 'stat',
    params: [{ label: '魔法上限比例', min: 1, max: 100, defaultVal: 10, unit: '%' }],
    format: (p1) => `MP MAX +${p1}%`
  },

  // --- 0x98 ~ 0x99 百分比攻强 ---
  0x98: {
    code: 0x98,
    name: '物理攻击力增加(百分比)',
    category: 'special',
    params: [{ label: '物攻百分比', min: 1, max: 100, defaultVal: 10, unit: '%' }],
    format: (p1) => `物理攻击力增加 +${p1}%`
  },
  0x99: {
    code: 0x99,
    name: '魔法攻击力增加(百分比)',
    category: 'special',
    params: [{ label: '魔攻百分比', min: 1, max: 100, defaultVal: 10, unit: '%' }],
    format: (p1) => `魔法攻击力增加 +${p1}%`
  },

  // --- 0x9A, 0x9B, 0x9F 收益类 (官方真实映射) ---
  0x9a: {
    code: 0x9a,
    name: '经验值获得增加',
    category: 'special',
    params: [{ label: '经验值比例', min: 1, max: 100, defaultVal: 20, unit: '%' }],
    format: (p1) => `经验值获得增加 +${p1}%`
  },
  0x9b: {
    code: 0x9b,
    name: '道具掉落率增加',
    category: 'special',
    params: [{ label: '掉落率比例', min: 1, max: 100, defaultVal: 20, unit: '%' }],
    format: (p1) => `道具掉落率增加 +${p1}%`
  },
  0x9f: {
    code: 0x9f,
    name: '金币获得量增加',
    category: 'special',
    params: [{ label: '金币增加比例', min: 1, max: 100, defaultVal: 10, unit: '%' }],
    format: (p1) => `金币获得量 +${p1}%`
  }
}

// 官方 NPC 莎兰附魔商店全量 106 种部位附魔预设
// 严格对齐 3.etc (NPC 附魔卡数据库) 原生 106 条记录，部位与数量完全一致：
// 武器(18种) + 肩膀(21种) + 上衣(18种) + 下装(17种) + 手腕(14种) + 戒指(18种) = 106种
export const ENCHANT_PRESETS: EnchantPreset[] = [
  {
    id: 'sharan_000_weapon',
    name: '💥 魔人丹特: 追加伤害 +5%',
    desc: '官方莎兰附魔(史诗) #1：魔人丹特的卡片-武器，部位：武器，效果：追加伤害 +5%',
    code: 0x5f,
    param1: 5,
    param2: 0,
    param3: 0,
    icon: '💥',
    parts: ['weapon'],
    quality: 'orange'
  },
  {
    id: 'sharan_001_weapon',
    name: '⚔️ 哥布林: 物理攻击力 +5',
    desc: '官方莎兰附魔(普通) #2：哥布林卡片，部位：武器，效果：物理攻击力 +5',
    code: 0x0a,
    param1: 5,
    param2: 0,
    param3: 0,
    icon: '⚔️',
    parts: ['weapon'],
    quality: 'white'
  },
  {
    id: 'sharan_002_weapon',
    name: '🔥 牛头巨兽: 赋予火属性攻击',
    desc: '官方莎兰附魔(普通) #3：牛头巨兽卡片，部位：武器，效果：赋予火属性攻击',
    code: 0x79,
    param1: 0,
    param2: 0,
    param3: 0,
    icon: '🔥',
    parts: ['weapon'],
    quality: 'white'
  },
  {
    id: 'sharan_003_weapon',
    name: '🔮 卡尔扎克: 魔法攻击力 +10',
    desc: '官方莎兰附魔(普通) #4：卡尔扎克卡片，部位：武器，效果：魔法攻击力 +10',
    code: 0x0b,
    param1: 10,
    param2: 0,
    param3: 0,
    icon: '🔮',
    parts: ['weapon'],
    quality: 'white'
  },
  {
    id: 'sharan_004_weapon',
    name: '⚔️ 驱逐者: 物理攻击力 +35',
    desc: '官方莎兰附魔(普通) #5：驱逐者卡片，部位：武器，效果：物理攻击力 +35',
    code: 0x0a,
    param1: 35,
    param2: 0,
    param3: 0,
    icon: '⚔️',
    parts: ['weapon'],
    quality: 'white'
  },
  {
    id: 'sharan_005_weapon',
    name: '🔮 龙头炮: 魔法攻击力 +39',
    desc: '官方莎兰附魔(普通) #6：龙头炮卡片，部位：武器，效果：魔法攻击力 +39',
    code: 0x0b,
    param1: 39,
    param2: 0,
    param3: 0,
    icon: '🔮',
    parts: ['weapon'],
    quality: 'white'
  },
  {
    id: 'sharan_006_weapon',
    name: '❄️ 瑟冥特克: 赋予冰属性攻击',
    desc: '官方莎兰附魔(普通) #7：瑟冥特克卡片，部位：武器，效果：赋予冰属性攻击',
    code: 0x7a,
    param1: 0,
    param2: 0,
    param3: 0,
    icon: '❄️',
    parts: ['weapon'],
    quality: 'white'
  },
  {
    id: 'sharan_007_weapon',
    name: '🔮 雷沃斯: 魔法攻击力 +15',
    desc: '官方莎兰附魔(普通) #8：雷沃斯卡片，部位：武器，效果：魔法攻击力 +15',
    code: 0x0b,
    param1: 15,
    param2: 0,
    param3: 0,
    icon: '🔮',
    parts: ['weapon'],
    quality: 'white'
  },
  {
    id: 'sharan_008_weapon',
    name: '⚔️ 班图战士: 物理攻击力 +51',
    desc: '官方莎兰附魔(普通) #9：班图战士卡片，部位：武器，效果：物理攻击力 +51',
    code: 0x0a,
    param1: 51,
    param2: 0,
    param3: 0,
    icon: '⚔️',
    parts: ['weapon'],
    quality: 'white'
  },
  {
    id: 'sharan_009_weapon',
    name: '🌑 园丁鲁尔: 赋予暗属性攻击',
    desc: '官方莎兰附魔(高级) #10：园丁鲁尔卡片，部位：武器，效果：赋予暗属性攻击',
    code: 0x7c,
    param1: 0,
    param2: 0,
    param3: 0,
    icon: '🌑',
    parts: ['weapon'],
    quality: 'blue'
  },
  {
    id: 'sharan_010_weapon',
    name: '⚔️ 霍克伊: 物理攻击力 +63',
    desc: '官方莎兰附魔(高级) #11：霍克伊卡片，部位：武器，效果：物理攻击力 +63',
    code: 0x0a,
    param1: 63,
    param2: 0,
    param3: 0,
    icon: '⚔️',
    parts: ['weapon'],
    quality: 'blue'
  },
  {
    id: 'sharan_011_weapon',
    name: '⚔️ 暗咒猫妖: 物理攻击力 +25',
    desc: '官方莎兰附魔(稀有) #12：暗咒猫妖卡片，部位：武器，效果：物理攻击力 +25',
    code: 0x0a,
    param1: 25,
    param2: 0,
    param3: 0,
    icon: '⚔️',
    parts: ['weapon'],
    quality: 'purple'
  },
  {
    id: 'sharan_012_weapon',
    name: '🔮 落雷凯诺: 魔法攻击力 +25',
    desc: '官方莎兰附魔(稀有) #13：落雷凯诺卡片，部位：武器，效果：魔法攻击力 +25',
    code: 0x0b,
    param1: 25,
    param2: 0,
    param3: 0,
    icon: '🔮',
    parts: ['weapon'],
    quality: 'purple'
  },
  {
    id: 'sharan_013_weapon',
    name: '⚡ 天之驱逐者: 赋予光属性攻击',
    desc: '官方莎兰附魔(稀有) #14：天之驱逐者卡片，部位：武器，效果：赋予光属性攻击',
    code: 0x7b,
    param1: 0,
    param2: 0,
    param3: 0,
    icon: '⚡',
    parts: ['weapon'],
    quality: 'purple'
  },
  {
    id: 'sharan_014_weapon',
    name: '💥 鲁乌格: 追加伤害 +126',
    desc: '官方莎兰附魔(稀有) #15：鲁乌格卡片，部位：武器，效果：追加伤害 +126',
    code: 0x0e,
    param1: 126,
    param2: 0,
    param3: 0,
    icon: '💥',
    parts: ['weapon'],
    quality: 'purple'
  },
  {
    id: 'sharan_015_weapon',
    name: '💥 白化哥布林: 追加伤害 +31',
    desc: '官方莎兰附魔(神器) #16：白化哥布林卡片，部位：武器，效果：追加伤害 +31',
    code: 0x0e,
    param1: 31,
    param2: 0,
    param3: 0,
    icon: '💥',
    parts: ['weapon'],
    quality: 'pink'
  },
  {
    id: 'sharan_016_weapon',
    name: '💥 过度生长: 追加伤害 +180',
    desc: '官方莎兰附魔(史诗) #17：过度生长卡片，部位：武器，效果：追加伤害 +180',
    code: 0x0e,
    param1: 180,
    param2: 0,
    param3: 0,
    icon: '💥',
    parts: ['weapon'],
    quality: 'orange'
  },
  {
    id: 'sharan_017_weapon',
    name: '💥 梦魔: 追加伤害 +180',
    desc: '官方莎兰附魔(史诗) #18：梦魔卡片，部位：武器，效果：追加伤害 +180',
    code: 0x0e,
    param1: 180,
    param2: 0,
    param3: 0,
    icon: '💥',
    parts: ['weapon'],
    quality: 'orange'
  },
  {
    id: 'sharan_018_shoulder',
    name: '🌟 魔人丹特: 全属性增加 +15',
    desc: '官方莎兰附魔(史诗) #19：魔人丹特的卡片-肩膀，部位：肩膀，效果：全属性增加 +15',
    code: 0x60,
    param1: 15,
    param2: 0,
    param3: 0,
    icon: '🌟',
    parts: ['shoulder'],
    quality: 'orange'
  },
  {
    id: 'sharan_019_shoulder',
    name: '🧠 赤哥布林: 智力 +2',
    desc: '官方莎兰附魔(普通) #20：赤哥布林卡片，部位：肩膀，效果：智力 +2',
    code: 0x02,
    param1: 2,
    param2: 0,
    param3: 0,
    icon: '🧠',
    parts: ['shoulder'],
    quality: 'white'
  },
  {
    id: 'sharan_020_shoulder',
    name: '💪 投掷十夫长: 力量 +3',
    desc: '官方莎兰附魔(普通) #21：投掷十夫长卡片，部位：肩膀，效果：力量 +3',
    code: 0x01,
    param1: 3,
    param2: 0,
    param3: 0,
    icon: '💪',
    parts: ['shoulder'],
    quality: 'white'
  },
  {
    id: 'sharan_021_shoulder',
    name: '🧠 猫妖: 智力 +2',
    desc: '官方莎兰附魔(普通) #22：猫妖卡片，部位：肩膀，效果：智力 +2',
    code: 0x02,
    param1: 2,
    param2: 0,
    param3: 0,
    icon: '🧠',
    parts: ['shoulder'],
    quality: 'white'
  },
  {
    id: 'sharan_022_shoulder',
    name: '🧠 悲鸣草: 智力 +2',
    desc: '官方莎兰附魔(普通) #23：悲鸣草卡片，部位：肩膀，效果：智力 +2',
    code: 0x02,
    param1: 2,
    param2: 0,
    param3: 0,
    icon: '🧠',
    parts: ['shoulder'],
    quality: 'white'
  },
  {
    id: 'sharan_023_shoulder',
    name: '💪 烈焰哥布林: 力量 +3',
    desc: '官方莎兰附魔(普通) #24：烈焰哥布林卡片，部位：肩膀，效果：力量 +3',
    code: 0x01,
    param1: 3,
    param2: 0,
    param3: 0,
    icon: '💪',
    parts: ['shoulder'],
    quality: 'white'
  },
  {
    id: 'sharan_024_shoulder',
    name: '💪 翼龙: 力量 +5',
    desc: '官方莎兰附魔(普通) #25：翼龙卡片，部位：肩膀，效果：力量 +5',
    code: 0x01,
    param1: 5,
    param2: 0,
    param3: 0,
    icon: '💪',
    parts: ['shoulder'],
    quality: 'white'
  },
  {
    id: 'sharan_025_shoulder',
    name: '💪 夜叉: 力量 +6',
    desc: '官方莎兰附魔(普通) #26：夜叉卡片，部位：肩膀，效果：力量 +6',
    code: 0x01,
    param1: 6,
    param2: 0,
    param3: 0,
    icon: '💪',
    parts: ['shoulder'],
    quality: 'white'
  },
  {
    id: 'sharan_026_shoulder',
    name: '🧠 嗜血猫妖: 智力 +8',
    desc: '官方莎兰附魔(普通) #27：嗜血猫妖卡片，部位：肩膀，效果：智力 +8',
    code: 0x02,
    param1: 8,
    param2: 0,
    param3: 0,
    icon: '🧠',
    parts: ['shoulder'],
    quality: 'white'
  },
  {
    id: 'sharan_027_shoulder',
    name: '💪 玩具士兵: 力量 +10',
    desc: '官方莎兰附魔(普通) #28：玩具士兵卡片，部位：肩膀，效果：力量 +10',
    code: 0x01,
    param1: 10,
    param2: 0,
    param3: 0,
    icon: '💪',
    parts: ['shoulder'],
    quality: 'white'
  },
  {
    id: 'sharan_028_shoulder',
    name: '🧠 图腾术士: 智力 +10',
    desc: '官方莎兰附魔(普通) #29：图腾术士卡片，部位：肩膀，效果：智力 +10',
    code: 0x02,
    param1: 10,
    param2: 0,
    param3: 0,
    icon: '🧠',
    parts: ['shoulder'],
    quality: 'white'
  },
  {
    id: 'sharan_029_shoulder',
    name: '💪 雪怪: 力量 +10',
    desc: '官方莎兰附魔(普通) #30：雪怪卡片，部位：肩膀，效果：力量 +10',
    code: 0x01,
    param1: 10,
    param2: 0,
    param3: 0,
    icon: '💪',
    parts: ['shoulder'],
    quality: 'white'
  },
  {
    id: 'sharan_030_shoulder',
    name: '💪 夏莱多: 力量 +10',
    desc: '官方莎兰附魔(高级) #31：夏莱多卡片，部位：肩膀，效果：力量 +10',
    code: 0x01,
    param1: 10,
    param2: 0,
    param3: 0,
    icon: '💪',
    parts: ['shoulder'],
    quality: 'blue'
  },
  {
    id: 'sharan_031_shoulder',
    name: '🧠 彼诺修: 智力 +13',
    desc: '官方莎兰附魔(稀有) #32：彼诺修卡片，部位：肩膀，效果：智力 +13',
    code: 0x02,
    param1: 13,
    param2: 0,
    param3: 0,
    icon: '🧠',
    parts: ['shoulder'],
    quality: 'purple'
  },
  {
    id: 'sharan_032_shoulder',
    name: '💪 黄金巨人普拉塔尼: 力量 +15',
    desc: '官方莎兰附魔(稀有) #33：黄金巨人普拉塔尼卡片，部位：肩膀，效果：力量 +15',
    code: 0x01,
    param1: 15,
    param2: 0,
    param3: 0,
    icon: '💪',
    parts: ['shoulder'],
    quality: 'purple'
  },
  {
    id: 'sharan_033_shoulder',
    name: '💪 巨型黑章鱼: 力量 +20',
    desc: '官方莎兰附魔(稀有) #34：巨型黑章鱼卡片，部位：肩膀，效果：力量 +20',
    code: 0x01,
    param1: 20,
    param2: 0,
    param3: 0,
    icon: '💪',
    parts: ['shoulder'],
    quality: 'purple'
  },
  {
    id: 'sharan_034_shoulder',
    name: '🧠 塞斯奇: 智力 +20',
    desc: '官方莎兰附魔(稀有) #35：塞斯奇卡片，部位：肩膀，效果：智力 +20',
    code: 0x02,
    param1: 20,
    param2: 0,
    param3: 0,
    icon: '🧠',
    parts: ['shoulder'],
    quality: 'purple'
  },
  {
    id: 'sharan_035_shoulder',
    name: '💪 白化萨乌塔: 力量 +15',
    desc: '官方莎兰附魔(神器) #36：白化萨乌塔卡片，部位：肩膀，效果：力量 +15',
    code: 0x01,
    param1: 15,
    param2: 0,
    param3: 0,
    icon: '💪',
    parts: ['shoulder'],
    quality: 'pink'
  },
  {
    id: 'sharan_036_shoulder',
    name: '🧠 不灭的守门将: 智力 +30',
    desc: '官方莎兰附魔(神器) #37：不灭的守门将卡片，部位：肩膀，效果：智力 +30',
    code: 0x02,
    param1: 30,
    param2: 0,
    param3: 0,
    icon: '🧠',
    parts: ['shoulder'],
    quality: 'pink'
  },
  {
    id: 'sharan_037_shoulder',
    name: '💪 波动之鲁卡斯: 力量 +20',
    desc: '官方莎兰附魔(神器) #38：波动之鲁卡斯卡片，部位：肩膀，效果：力量 +20',
    code: 0x01,
    param1: 20,
    param2: 0,
    param3: 0,
    icon: '💪',
    parts: ['shoulder'],
    quality: 'pink'
  },
  {
    id: 'sharan_038_shoulder',
    name: '🧠 哈巴斯: 智力 +40',
    desc: '官方莎兰附魔(史诗) #39：哈巴斯卡片，部位：肩膀，效果：智力 +40',
    code: 0x02,
    param1: 40,
    param2: 0,
    param3: 0,
    icon: '🧠',
    parts: ['shoulder'],
    quality: 'orange'
  },
  {
    id: 'sharan_039_top',
    name: '💎 魔人丹特: HP MAX +10%',
    desc: '官方莎兰附魔(史诗) #40：魔人丹特的卡片-上衣，部位：上衣，效果：HP MAX +10%',
    code: 0x7e,
    param1: 10,
    param2: 0,
    param3: 0,
    icon: '💎',
    parts: ['top'],
    quality: 'orange'
  },
  {
    id: 'sharan_040_top',
    name: '💎 魔人丹特: MP MAX +10%',
    desc: '官方莎兰附魔(史诗) #41：魔人丹特的卡片-上衣，部位：上衣，效果：MP MAX +10%',
    code: 0x7f,
    param1: 10,
    param2: 0,
    param3: 0,
    icon: '💎',
    parts: ['top'],
    quality: 'orange'
  },
  {
    id: 'sharan_041_top',
    name: '🧪 毒猫王: MP MAX +45',
    desc: '官方莎兰附魔(普通) #42：毒猫王卡片，部位：上衣，效果：MP MAX +45',
    code: 0x06,
    param1: 45,
    param2: 0,
    param3: 0,
    icon: '🧪',
    parts: ['top'],
    quality: 'white'
  },
  {
    id: 'sharan_042_top',
    name: '💖 丛林僵尸: HP每分恢复 +3',
    desc: '官方莎兰附魔(普通) #43：丛林僵尸卡片，部位：上衣，效果：HP每分恢复 +3',
    code: 0x07,
    param1: 3,
    param2: 0,
    param3: 0,
    icon: '💖',
    parts: ['top'],
    quality: 'white'
  },
  {
    id: 'sharan_043_top',
    name: '💖 米尼乌斯: HP每分恢复 +5',
    desc: '官方莎兰附魔(普通) #44：米尼乌斯卡片，部位：上衣，效果：HP每分恢复 +5',
    code: 0x07,
    param1: 5,
    param2: 0,
    param3: 0,
    icon: '💖',
    parts: ['top'],
    quality: 'white'
  },
  {
    id: 'sharan_044_top',
    name: '💖 剑士: HP每分恢复 +6',
    desc: '官方莎兰附魔(普通) #45：剑士卡片，部位：上衣，效果：HP每分恢复 +6',
    code: 0x07,
    param1: 6,
    param2: 0,
    param3: 0,
    icon: '💖',
    parts: ['top'],
    quality: 'white'
  },
  {
    id: 'sharan_045_top',
    name: '🧪 GBL教信徒: MP MAX +65',
    desc: '官方莎兰附魔(普通) #46：GBL教信徒卡片，部位：上衣，效果：MP MAX +65',
    code: 0x06,
    param1: 65,
    param2: 0,
    param3: 0,
    icon: '🧪',
    parts: ['top'],
    quality: 'white'
  },
  {
    id: 'sharan_046_top',
    name: '🩸 复活的信徒: HP MAX +80',
    desc: '官方莎兰附魔(普通) #47：复活的信徒卡片，部位：上衣，效果：HP MAX +80',
    code: 0x05,
    param1: 80,
    param2: 0,
    param3: 0,
    icon: '🩸',
    parts: ['top'],
    quality: 'white'
  },
  {
    id: 'sharan_047_top',
    name: '🧪 班图女战士: MP MAX +90',
    desc: '官方莎兰附魔(普通) #48：班图女战士卡片，部位：上衣，效果：MP MAX +90',
    code: 0x06,
    param1: 90,
    param2: 0,
    param3: 0,
    icon: '🧪',
    parts: ['top'],
    quality: 'white'
  },
  {
    id: 'sharan_048_top',
    name: '💖 图卢斯族: HP每分恢复 +9',
    desc: '官方莎兰附魔(普通) #49：图卢斯族卡片，部位：上衣，效果：HP每分恢复 +9',
    code: 0x07,
    param1: 9,
    param2: 0,
    param3: 0,
    icon: '💖',
    parts: ['top'],
    quality: 'white'
  },
  {
    id: 'sharan_049_top',
    name: '🧪 寒冰蜘蛛: MP每分恢复 +9',
    desc: '官方莎兰附魔(普通) #50：寒冰蜘蛛卡片，部位：上衣，效果：MP每分恢复 +9',
    code: 0x08,
    param1: 9,
    param2: 0,
    param3: 0,
    icon: '🧪',
    parts: ['top'],
    quality: 'white'
  },
  {
    id: 'sharan_050_top',
    name: '💖 艾力斯: HP每分恢复 +7',
    desc: '官方莎兰附魔(高级) #51：艾力斯卡片，部位：上衣，效果：HP每分恢复 +7',
    code: 0x07,
    param1: 7,
    param2: 0,
    param3: 0,
    icon: '💖',
    parts: ['top'],
    quality: 'blue'
  },
  {
    id: 'sharan_051_top',
    name: '🧪 拉比纳: MP每分恢复 +8',
    desc: '官方莎兰附魔(高级) #52：拉比纳卡片，部位：上衣，效果：MP每分恢复 +8',
    code: 0x08,
    param1: 8,
    param2: 0,
    param3: 0,
    icon: '🧪',
    parts: ['top'],
    quality: 'blue'
  },
  {
    id: 'sharan_052_top',
    name: '💖 鲁卡斯: HP/MP每分恢复 +5',
    desc: '官方莎兰附魔(稀有) #53：鲁卡斯卡片，部位：上衣，效果：HP/MP每分恢复 +5',
    code: 0x09,
    param1: 5,
    param2: 0,
    param3: 0,
    icon: '💖',
    parts: ['top'],
    quality: 'purple'
  },
  {
    id: 'sharan_053_top',
    name: '💖 罪恶之眼: HP/MP每分恢复 +6',
    desc: '官方莎兰附魔(稀有) #54：罪恶之眼卡片，部位：上衣，效果：HP/MP每分恢复 +6',
    code: 0x09,
    param1: 6,
    param2: 0,
    param3: 0,
    icon: '💖',
    parts: ['top'],
    quality: 'purple'
  },
  {
    id: 'sharan_054_top',
    name: '💖 偏强的哈尼克: HP/MP每分恢复 +10',
    desc: '官方莎兰附魔(神器) #55：偏强的哈尼克卡片，部位：上衣，效果：HP/MP每分恢复 +10',
    code: 0x09,
    param1: 10,
    param2: 0,
    param3: 0,
    icon: '💖',
    parts: ['top'],
    quality: 'pink'
  },
  {
    id: 'sharan_055_top',
    name: '💖 烈焰: HP/MP每分恢复 +15',
    desc: '官方莎兰附魔(史诗) #56：烈焰卡片，部位：上衣，效果：HP/MP每分恢复 +15',
    code: 0x09,
    param1: 15,
    param2: 0,
    param3: 0,
    icon: '💖',
    parts: ['top'],
    quality: 'orange'
  },
  {
    id: 'sharan_056_top',
    name: '🩸 格拉瑟: HP MAX +200',
    desc: '官方莎兰附魔(史诗) #57：格拉瑟卡片，部位：上衣，效果：HP MAX +200',
    code: 0x05,
    param1: 200,
    param2: 0,
    param3: 0,
    icon: '🩸',
    parts: ['top'],
    quality: 'orange'
  },
  {
    id: 'sharan_057_bottom',
    name: '💖 魔人丹特: HP/MP每分恢复 +5',
    desc: '官方莎兰附魔(史诗) #58：魔人丹特的卡片-下装，部位：下装，效果：HP/MP每分恢复 +5',
    code: 0x09,
    param1: 5,
    param2: 0,
    param3: 0,
    icon: '💖',
    parts: ['bottom'],
    quality: 'orange'
  },
  {
    id: 'sharan_058_bottom',
    name: '🏃 萤光猫妖: 移动速度 +1%',
    desc: '官方莎兰附魔(普通) #59：萤光猫妖卡片，部位：下装，效果：移动速度 +1%',
    code: 0x11,
    param1: 1,
    param2: 0,
    param3: 0,
    icon: '🏃',
    parts: ['bottom'],
    quality: 'white'
  },
  {
    id: 'sharan_059_bottom',
    name: '🧘 石巨人: 精神 +3',
    desc: '官方莎兰附魔(普通) #60：石巨人卡片，部位：下装，效果：精神 +3',
    code: 0x04,
    param1: 3,
    param2: 0,
    param3: 0,
    icon: '🧘',
    parts: ['bottom'],
    quality: 'white'
  },
  {
    id: 'sharan_060_bottom',
    name: '🛡️ 章鱼怪: 体力 +6',
    desc: '官方莎兰附魔(普通) #61：章鱼怪卡片，部位：下装，效果：体力 +6',
    code: 0x03,
    param1: 6,
    param2: 0,
    param3: 0,
    icon: '🛡️',
    parts: ['bottom'],
    quality: 'white'
  },
  {
    id: 'sharan_061_bottom',
    name: '🧘 寒冰虎: 精神 +8',
    desc: '官方莎兰附魔(普通) #62：寒冰虎卡片，部位：下装，效果：精神 +8',
    code: 0x04,
    param1: 8,
    param2: 0,
    param3: 0,
    icon: '🧘',
    parts: ['bottom'],
    quality: 'white'
  },
  {
    id: 'sharan_062_bottom',
    name: '🧘 斯卡萨幼龙: 精神 +8',
    desc: '官方莎兰附魔(普通) #63：斯卡萨幼龙卡片，部位：下装，效果：精神 +8',
    code: 0x04,
    param1: 8,
    param2: 0,
    param3: 0,
    icon: '🧘',
    parts: ['bottom'],
    quality: 'white'
  },
  {
    id: 'sharan_063_bottom',
    name: '🏃 普拉格: 移动速度 +2%',
    desc: '官方莎兰附魔(高级) #64：普拉格卡片，部位：下装，效果：移动速度 +2%',
    code: 0x11,
    param1: 2,
    param2: 0,
    param3: 0,
    icon: '🏃',
    parts: ['bottom'],
    quality: 'blue'
  },
  {
    id: 'sharan_064_bottom',
    name: '🧘 阿克雄: 精神 +5',
    desc: '官方莎兰附魔(高级) #65：阿克雄卡片，部位：下装，效果：精神 +5',
    code: 0x04,
    param1: 5,
    param2: 0,
    param3: 0,
    icon: '🧘',
    parts: ['bottom'],
    quality: 'blue'
  },
  {
    id: 'sharan_065_bottom',
    name: '🏃 敏捷的拉尔: 移动速度 +3%',
    desc: '官方莎兰附魔(稀有) #66：敏捷的拉尔卡片，部位：下装，效果：移动速度 +3%',
    code: 0x11,
    param1: 3,
    param2: 0,
    param3: 0,
    icon: '🏃',
    parts: ['bottom'],
    quality: 'purple'
  },
  {
    id: 'sharan_066_bottom',
    name: '🏃 疯狂伊凡: 移动速度 +4%',
    desc: '官方莎兰附魔(稀有) #67：疯狂伊凡卡片，部位：下装，效果：移动速度 +4%',
    code: 0x11,
    param1: 4,
    param2: 0,
    param3: 0,
    icon: '🏃',
    parts: ['bottom'],
    quality: 'purple'
  },
  {
    id: 'sharan_067_bottom',
    name: '🛡️ 牛头王萨乌塔: 体力 +7',
    desc: '官方莎兰附魔(神器) #68：牛头王萨乌塔卡片，部位：下装，效果：体力 +7',
    code: 0x03,
    param1: 7,
    param2: 0,
    param3: 0,
    icon: '🛡️',
    parts: ['bottom'],
    quality: 'pink'
  },
  {
    id: 'sharan_068_bottom',
    name: '🛡️ 赛格哈特: 体力 +12',
    desc: '官方莎兰附魔(神器) #69：赛格哈特卡片，部位：下装，效果：体力 +12',
    code: 0x03,
    param1: 12,
    param2: 0,
    param3: 0,
    icon: '🛡️',
    parts: ['bottom'],
    quality: 'pink'
  },
  {
    id: 'sharan_069_bottom',
    name: '🧘 红炎之翼龙: 精神 +16',
    desc: '官方莎兰附魔(神器) #70：红炎之翼龙卡片，部位：下装，效果：精神 +16',
    code: 0x04,
    param1: 16,
    param2: 0,
    param3: 0,
    icon: '🧘',
    parts: ['bottom'],
    quality: 'pink'
  },
  {
    id: 'sharan_070_bottom',
    name: '🏃 多尼尔EX: 移动速度 +5%',
    desc: '官方莎兰附魔(神器) #71：多尼尔EX卡片，部位：下装，效果：移动速度 +5%',
    code: 0x11,
    param1: 5,
    param2: 0,
    param3: 0,
    icon: '🏃',
    parts: ['bottom'],
    quality: 'pink'
  },
  {
    id: 'sharan_071_bottom',
    name: '🏃 冰龙斯卡萨: 移动速度 +6%',
    desc: '官方莎兰附魔(神器) #72：冰龙斯卡萨卡片，部位：下装，效果：移动速度 +6%',
    code: 0x11,
    param1: 6,
    param2: 0,
    param3: 0,
    icon: '🏃',
    parts: ['bottom'],
    quality: 'pink'
  },
  {
    id: 'sharan_072_bottom',
    name: '🛡️ 寒冰巨人利库: 体力 +14',
    desc: '官方莎兰附魔(神器) #73：寒冰巨人利库卡片，部位：下装，效果：体力 +14',
    code: 0x03,
    param1: 14,
    param2: 0,
    param3: 0,
    icon: '🛡️',
    parts: ['bottom'],
    quality: 'pink'
  },
  {
    id: 'sharan_073_bottom',
    name: '🛡️ 卡塔利亚: 体力 +20',
    desc: '官方莎兰附魔(史诗) #74：卡塔利亚卡片，部位：下装，效果：体力 +20',
    code: 0x03,
    param1: 20,
    param2: 0,
    param3: 0,
    icon: '🛡️',
    parts: ['bottom'],
    quality: 'orange'
  },
  {
    id: 'sharan_074_bracelet',
    name: '🛡️ 魔人丹特: 所有属性抗性 +15',
    desc: '官方莎兰附魔(史诗) #75：魔人丹特的卡片-手腕，部位：手腕，效果：所有属性抗性 +15',
    code: 0x65,
    param1: 15,
    param2: 0,
    param3: 0,
    icon: '🛡️',
    parts: ['bracelet'],
    quality: 'orange'
  },
  {
    id: 'sharan_075_bracelet',
    name: '⚡ 十夫长: 光属性抗性 +3',
    desc: '官方莎兰附魔(普通) #76：十夫长卡片，部位：手腕，效果：光属性抗性 +3',
    code: 0x16,
    param1: 3,
    param2: 3,
    param3: 0,
    icon: '⚡',
    parts: ['bracelet'],
    quality: 'white'
  },
  {
    id: 'sharan_076_bracelet',
    name: '🌑 青哥布林: 暗属性抗性 +3',
    desc: '官方莎兰附魔(普通) #77：青哥布林卡片，部位：手腕，效果：暗属性抗性 +3',
    code: 0x17,
    param1: 3,
    param2: 3,
    param3: 0,
    icon: '🌑',
    parts: ['bracelet'],
    quality: 'white'
  },
  {
    id: 'sharan_077_bracelet',
    name: '⚡ 人偶师: 光属性抗性 +4',
    desc: '官方莎兰附魔(普通) #78：人偶师卡片，部位：手腕，效果：光属性抗性 +4',
    code: 0x16,
    param1: 4,
    param2: 4,
    param3: 0,
    icon: '⚡',
    parts: ['bracelet'],
    quality: 'white'
  },
  {
    id: 'sharan_078_bracelet',
    name: '🔥 树精: 火属性抗性 +6',
    desc: '官方莎兰附魔(普通) #79：树精卡片，部位：手腕，效果：火属性抗性 +6',
    code: 0x14,
    param1: 6,
    param2: 6,
    param3: 0,
    icon: '🔥',
    parts: ['bracelet'],
    quality: 'white'
  },
  {
    id: 'sharan_079_bracelet',
    name: '🌑 实验体: 暗属性抗性 +20',
    desc: '官方莎兰附魔(普通) #80：实验体卡片，部位：手腕，效果：暗属性抗性 +20',
    code: 0x17,
    param1: 20,
    param2: 0,
    param3: 0,
    icon: '🌑',
    parts: ['bracelet'],
    quality: 'white'
  },
  {
    id: 'sharan_080_bracelet',
    name: '❄️ 石刃拉泽尔: 冰属性抗性 +5',
    desc: '官方莎兰附魔(高级) #81：石刃拉泽尔卡片，部位：手腕，效果：冰属性抗性 +5',
    code: 0x15,
    param1: 5,
    param2: 5,
    param3: 0,
    icon: '❄️',
    parts: ['bracelet'],
    quality: 'blue'
  },
  {
    id: 'sharan_081_bracelet',
    name: '⚡ 瓦皮埃拉: 光属性抗性 +8',
    desc: '官方莎兰附魔(稀有) #82：瓦皮埃拉卡片，部位：手腕，效果：光属性抗性 +8',
    code: 0x16,
    param1: 8,
    param2: 8,
    param3: 0,
    icon: '⚡',
    parts: ['bracelet'],
    quality: 'purple'
  },
  {
    id: 'sharan_082_bracelet',
    name: '❄️ 克拉赫: 冰属性抗性 +10',
    desc: '官方莎兰附魔(神器) #83：克拉赫卡片，部位：手腕，效果：冰属性抗性 +10',
    code: 0x15,
    param1: 10,
    param2: 10,
    param3: 0,
    icon: '❄️',
    parts: ['bracelet'],
    quality: 'pink'
  },
  {
    id: 'sharan_083_bracelet',
    name: '🛡️ 盗尸者骨狱息: 所有属性抗性 +3',
    desc: '官方莎兰附魔(神器) #84：盗尸者骨狱息卡片，部位：手腕，效果：所有属性抗性 +3',
    code: 0x65,
    param1: 3,
    param2: 3,
    param3: 0,
    icon: '🛡️',
    parts: ['bracelet'],
    quality: 'pink'
  },
  {
    id: 'sharan_084_bracelet',
    name: '🛡️ 马塞尔: 所有属性抗性 +6',
    desc: '官方莎兰附魔(神器) #85：马塞尔卡片，部位：手腕，效果：所有属性抗性 +6',
    code: 0x65,
    param1: 6,
    param2: 6,
    param3: 0,
    icon: '🛡️',
    parts: ['bracelet'],
    quality: 'pink'
  },
  {
    id: 'sharan_085_bracelet',
    name: '🛡️ 查理: 所有属性抗性 +8',
    desc: '官方莎兰附魔(神器) #86：查理卡片，部位：手腕，效果：所有属性抗性 +8',
    code: 0x65,
    param1: 8,
    param2: 8,
    param3: 0,
    icon: '🛡️',
    parts: ['bracelet'],
    quality: 'pink'
  },
  {
    id: 'sharan_086_bracelet',
    name: '🛡️ 奥尔卡: 所有属性抗性 +10',
    desc: '官方莎兰附魔(神器) #87：奥尔卡卡片，部位：手腕，效果：所有属性抗性 +10',
    code: 0x65,
    param1: 10,
    param2: 10,
    param3: 0,
    icon: '🛡️',
    parts: ['bracelet'],
    quality: 'pink'
  },
  {
    id: 'sharan_087_bracelet',
    name: '🛡️ 蒸汽: 所有属性抗性 +12',
    desc: '官方莎兰附魔(史诗) #88：蒸汽卡片，部位：手腕，效果：所有属性抗性 +12',
    code: 0x65,
    param1: 12,
    param2: 12,
    param3: 0,
    icon: '🛡️',
    parts: ['bracelet'],
    quality: 'orange'
  },
  {
    id: 'sharan_088_ring',
    name: '🎯 魔人丹特: 暴击率 +10%',
    desc: '官方莎兰附魔(史诗) #89：魔人丹特的卡片-戒指，部位：戒指，效果：暴击率 +10%',
    code: 0x0f,
    param1: 10,
    param2: 0,
    param3: 0,
    icon: '🎯',
    parts: ['ring'],
    quality: 'orange'
  },
  {
    id: 'sharan_089_ring',
    name: '⚡ 魔人丹特: 硬直恢复 +60',
    desc: '官方莎兰附魔(史诗) #90：魔人丹特的卡片-戒指，部位：戒指，效果：硬直恢复 +60',
    code: 0x10,
    param1: 60,
    param2: 0,
    param3: 0,
    icon: '⚡',
    parts: ['ring'],
    quality: 'orange'
  },
  {
    id: 'sharan_090_ring',
    name: '🎯 魔人丹特: 命中率 +8%',
    desc: '官方莎兰附魔(史诗) #91：魔人丹特的卡片-戒指，部位：戒指，效果：命中率 +8%',
    code: 0x12,
    param1: 8,
    param2: 0,
    param3: 0,
    icon: '🎯',
    parts: ['ring'],
    quality: 'orange'
  },
  {
    id: 'sharan_091_ring',
    name: '🍃 魔人丹特: 回避率 +8%',
    desc: '官方莎兰附魔(史诗) #92：魔人丹特的卡片-戒指，部位：戒指，效果：回避率 +8%',
    code: 0x13,
    param1: 8,
    param2: 0,
    param3: 0,
    icon: '🍃',
    parts: ['ring'],
    quality: 'orange'
  },
  {
    id: 'sharan_092_ring',
    name: '🎯 冰霜哥布林: 命中率 +2%',
    desc: '官方莎兰附魔(普通) #93：冰霜哥布林卡片，部位：戒指，效果：命中率 +2%',
    code: 0x12,
    param1: 2,
    param2: 0,
    param3: 0,
    icon: '🎯',
    parts: ['ring'],
    quality: 'white'
  },
  {
    id: 'sharan_093_ring',
    name: '🎯 牛头护卫: 暴击率 +3%',
    desc: '官方莎兰附魔(普通) #94：牛头护卫卡片，部位：戒指，效果：暴击率 +3%',
    code: 0x0f,
    param1: 3,
    param2: 0,
    param3: 0,
    icon: '🎯',
    parts: ['ring'],
    quality: 'white'
  },
  {
    id: 'sharan_094_ring',
    name: '🍃 饥饿僵户: 回避率 +2%',
    desc: '官方莎兰附魔(普通) #95：饥饿僵户卡片，部位：戒指，效果：回避率 +2%',
    code: 0x13,
    param1: 2,
    param2: 0,
    param3: 0,
    icon: '🍃',
    parts: ['ring'],
    quality: 'white'
  },
  {
    id: 'sharan_095_ring',
    name: '⚡ 多尼尔: 硬直恢复 +20',
    desc: '官方莎兰附魔(普通) #96：多尼尔卡片，部位：戒指，效果：硬直恢复 +20',
    code: 0x10,
    param1: 20,
    param2: 0,
    param3: 0,
    icon: '⚡',
    parts: ['ring'],
    quality: 'white'
  },
  {
    id: 'sharan_096_ring',
    name: '⚡ 蛇巫士: 硬直恢复 +25',
    desc: '官方莎兰附魔(普通) #97：蛇巫士卡片，部位：戒指，效果：硬直恢复 +25',
    code: 0x10,
    param1: 25,
    param2: 0,
    param3: 0,
    icon: '⚡',
    parts: ['ring'],
    quality: 'white'
  },
  {
    id: 'sharan_097_ring',
    name: '🍃 光环猎人: 回避率 +3%',
    desc: '官方莎兰附魔(高级) #98：光环猎人卡片，部位：戒指，效果：回避率 +3%',
    code: 0x13,
    param1: 3,
    param2: 0,
    param3: 0,
    icon: '🍃',
    parts: ['ring'],
    quality: 'blue'
  },
  {
    id: 'sharan_098_ring',
    name: '🍃 特鲁门: 回避率 +4%',
    desc: '官方莎兰附魔(高级) #99：特鲁门卡片，部位：戒指，效果：回避率 +4%',
    code: 0x13,
    param1: 4,
    param2: 0,
    param3: 0,
    icon: '🍃',
    parts: ['ring'],
    quality: 'blue'
  },
  {
    id: 'sharan_099_ring',
    name: '⚡ 白化猫妖: 硬直恢复 +30',
    desc: '官方莎兰附魔(稀有) #100：白化猫妖卡片，部位：戒指，效果：硬直恢复 +30',
    code: 0x10,
    param1: 30,
    param2: 0,
    param3: 0,
    icon: '⚡',
    parts: ['ring'],
    quality: 'purple'
  },
  {
    id: 'sharan_100_ring',
    name: '⚡ 大族长: 硬直恢复 +40',
    desc: '官方莎兰附魔(稀有) #101：大族长卡片，部位：戒指，效果：硬直恢复 +40',
    code: 0x10,
    param1: 40,
    param2: 0,
    param3: 0,
    icon: '⚡',
    parts: ['ring'],
    quality: 'purple'
  },
  {
    id: 'sharan_101_ring',
    name: '🎯 道格里: 命中率 +4%',
    desc: '官方莎兰附魔(神器) #102：道格里卡片，部位：戒指，效果：命中率 +4%',
    code: 0x12,
    param1: 4,
    param2: 0,
    param3: 0,
    icon: '🎯',
    parts: ['ring'],
    quality: 'pink'
  },
  {
    id: 'sharan_102_ring',
    name: '🎯 大主教: 命中率 +4%',
    desc: '官方莎兰附魔(神器) #103：大主教卡片，部位：戒指，效果：命中率 +4%',
    code: 0x12,
    param1: 4,
    param2: 0,
    param3: 0,
    icon: '🎯',
    parts: ['ring'],
    quality: 'pink'
  },
  {
    id: 'sharan_103_ring',
    name: '🎯 夜叉王: 暴击率 +4%',
    desc: '官方莎兰附魔(神器) #104：夜叉王，部位：戒指，效果：暴击率 +4%',
    code: 0x0f,
    param1: 4,
    param2: 0,
    param3: 0,
    icon: '🎯',
    parts: ['ring'],
    quality: 'pink'
  },
  {
    id: 'sharan_104_ring',
    name: '🎯 牛头械王: 暴击率 +5%',
    desc: '官方莎兰附魔(神器) #105：牛头械王卡片，部位：戒指，效果：暴击率 +5%',
    code: 0x0f,
    param1: 5,
    param2: 0,
    param3: 0,
    icon: '🎯',
    parts: ['ring'],
    quality: 'pink'
  },
  {
    id: 'sharan_105_ring',
    name: '🎯 梅塔西斯: 暴击率 +6%',
    desc: '官方莎兰附魔(史诗) #106：梅塔西斯卡片，部位：戒指，效果：暴击率 +6%',
    code: 0x0f,
    param1: 6,
    param2: 0,
    param3: 0,
    icon: '🎯',
    parts: ['ring'],
    quality: 'orange'
  }
]

/**
 * 根据物品 typeId 获取装备部位 (用于附魔卡片适用部位过滤)
 */
export function getEquipPartByTypeId(typeId: number): EquipPart | null {
  if (typeId >= 0x00 && typeId <= 0x03) return 'weapon' // 短剑、太刀、巨剑、光剑
  if (typeId === 0x04) return 'shoulder' // 护肩
  if (typeId === 0x05) return 'top'      // 上衣
  if (typeId === 0x06) return 'bottom'   // 下衣
  if (typeId === 0x07) return 'bracelet' // 手腕
  if (typeId === 0x08) return 'ring'     // 戒指
  return null
}

/**
 * 获取部位中文显示名称
 */
export function getEquipPartName(part: EquipPart | null): string {
  switch (part) {
    case 'weapon': return '武器'
    case 'shoulder': return '肩膀'
    case 'top': return '上衣'
    case 'bottom': return '下装'
    case 'bracelet': return '手腕'
    case 'ring': return '戒指'
    default: return '装备'
  }
}

/**
 * 根据部位过滤适用的附魔预设
 */
export function getEnchantPresetsForPart(part: EquipPart | null): EnchantPreset[] {
  if (!part) return ENCHANT_PRESETS
  return ENCHANT_PRESETS.filter(p => !p.parts || p.parts.includes(part))
}

/**
 * 格式化附魔说明文本
 */
export function formatEnchantText(code: number, p1: number = 0, p2: number = 0, p3: number = 0): string {
  if (!code || code === 0) return '无附魔'
  const def = ENCHANT_DEFINITIONS[code]
  if (def) {
    return def.format(p1, p2, p3)
  }
  // 未知或未定义代码通用格式化
  if (p3 > 0) {
    return `附魔 [0x${code.toString(16).padStart(2, '0').toUpperCase()}]: ${p1}, ${p2}, ${p3}`
  } else if (p2 > 0) {
    return `附魔 [0x${code.toString(16).padStart(2, '0').toUpperCase()}]: ${p1}, ${p2}`
  } else if (p1 > 0) {
    return `附魔 [0x${code.toString(16).padStart(2, '0').toUpperCase()}]: +${p1}`
  }
  return `附魔 [0x${code.toString(16).padStart(2, '0').toUpperCase()}]`
}

/**
 * 校验并限制词条参数数值：
 * 如果输入超过该参数的上限限制，则自动视为最大值 (max)；
 * 如果小于最小值，则视为最小值 (min)；
 * 物理硬上限严格锁死在 uint16 (0 ~ 65535)。
 */
export function clampEnchantParam(code: number, paramIndex: number, val: number): number {
  if (val === undefined || val === null || isNaN(val)) return 0
  const def = ENCHANT_DEFINITIONS[code]
  const intVal = Math.floor(val)
  if (!def || !def.params || !def.params[paramIndex]) {
    return Math.max(0, Math.min(65535, intVal))
  }
  const pDef = def.params[paramIndex]
  const effectiveMax = Math.min(65535, pDef.max)
  if (intVal > effectiveMax) {
    return effectiveMax
  }
  if (intVal < pDef.min) {
    return pDef.min
  }
  return intVal
}
